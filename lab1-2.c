...
#define _XTAL_FREQ      1000000  //default clock speed set in OSCCON
#define Switch_Pin      PORTBbits.RB0
#define DetectsInARow   5 //for switch debouncing
...
void main (void)
{
    ...
    while (1) {
        ...
        Switch_Count = 5;
        do { // monitor switch input for 5 lows in a row to debounce
            if (Switch_Pin == 0) { // pressed state detected
                Switch_Count++;
            }
            else {
                Switch_Count = 0;
            }   
            __delay_ms(1);    // delay 250 cycles or 1ms.
